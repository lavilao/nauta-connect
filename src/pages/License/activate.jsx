...
const Activate = ({ ...props }) => {
  ...
  const checkLicense = (event, token = license) => {
    ...
    chrome.storage.sync.get(['identity'], ({ identity: i, ...v }) => {
      try {
        const identityCheck = jwt.verify(token, PUBLIC_KEY, {
          algorithms: ['RS256'],
        });
        if (
          parseInt(identityCheck.id) !== i.id ||
          parseInt(identityCheck.timeCheck) !== i.timeCheck ||
          identityCheck.client.toLowerCase() !== i.client.toLowerCase()
        ) {
          setMsgLicense(
            'Su licencia no es válida o ya se ha vencido. Solicite una nueva licencia.'
          );
          return;
        }
        setMsgLicense(
          `Su licencia de Nauta Connect es válida hasta el ${new Date(identityCheck.exp * 1000).toLocaleString()}.`
        );
  ...
  return (
    <Paper className={classes.paper}>
      <Typography align="center">Licencia</Typography>
      {checking ? (
